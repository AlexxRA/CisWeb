...
if(isset($_GET['action']) == 'delete'){
    ...
    $queryCom = mysqli_query($conn, "SELECT * FROM comentarios WHERE identificador='$id_delete' and tabla='sector'");
    ...
    if(mysqli_num_rows($query) == 0){
        ...
        if($delete){
            $deleteCom = mysqli_query($conn, "DELETE FROM comentarios WHERE identificador='$id_delete' and tabla='sector'");
            if($deleteCom){
                mysqli_commit($conn);
...
if (isset($_GET["e"])){
    ...
    if($error==1){
        ...
        <script type="text/javascript">
            history.pushState(null, "", "showSector.php");
        </script>
    <?php
    }
    elseif($error==2){
