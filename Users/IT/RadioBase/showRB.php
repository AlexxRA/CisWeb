...
if(isset($_GET['action']) == 'delete'){
    ...
    $queryCom = mysqli_query($conn, "SELECT * FROM comentarios WHERE identificador='$id_delete' and tabla='radiobase'");
    ...
    if(mysqli_num_rows($query) == 0 ){
        ...
        if($delete){
            $deleteCom = mysqli_query($conn, "DELETE FROM comentarios WHERE identificador='$id_delete' and tabla='radiobase'");
            if($deleteCom){
                mysqli_commit($conn);
                header("Location: showRB.php?e=1");
            }
            else{
                mysqli_rollback($conn);
                header("Location: showRB.php?e=0");
            }
        }else{
            $error = mysqli_errno($conn);
            mysqli_rollback($conn);
            if($error==1451){
                header("Location: showRB.php?e=4");
            }
            else{
                header("Location: showRB.php?e=0");
            }
...
if (isset($_GET["e"])){
    ...
    if($error==1){
        ...
        <script type="text/javascript">
            history.pushState(null, "", "showRB.php");
        </script>
    <?php
    }
    elseif($error==2){
    ...
    elseif($error==4){
    echo "<div class='alert alert-danger alert-dismissable mb-0'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Error, no se puede eliminar porque la Radiobase tiene algún suscriptor asociado</div>";
    ?>
        <script type="text/javascript">
...
<script src="dataTableRB.js"></script>
